...
- Actual Supabase schema (column types, FKs, RLS policies) — not read; agent was instructed not to touch the DB connection.
- Whether `/posts/[id]` route is planned (linked but absent).
- GOTY voting model (community vs editorial) and tier-list persistence.

## Keeping docs fresh
Bump `last_verified_against_commit` in a file's front-matter when you re-verify it. If a module's source changed after that commit, treat its doc as stale until re-checked.

...
| `/streams`              | `pages/streams/index.astro`        | Monthly streams calendar (LUN-DOM grid, day boxes, stacked stream indicators) + responsive aside detail drawer (`StreamDetailDrawer` with multi-stream tabs and stacked views) | `loadStreamsForCalendar` via `get_streams_by_date_range` RPC (server)                                  |
| `/streams/[id]`         | `pages/streams/[id].astro`         | Standalone stream detail page (hero metadata, broadcast timing, sibling streams navigation bar for multi-stream days, chronological category & game timeline)                  | `loadStreamById` via `get_stream_by_id` RPC + sibling stream check (server)                            |
...
| `StreamDayBox.astro`            | Individual calendar cell: day number, "Hoy" highlight, stream status badge (stacked durations or live badges for multiple streams per day), category/game covers glimpse across all emissions, and click handler opening the aside drawer. |
| `StreamDetailDrawer.astro`      | Responsive aside detail drawer: mobile off-canvas drawer with backdrop, desktop slide-over panel, broadcast timings, multiple stream tabs/stacked views for multi-stream days, segment intervals, and link to `/streams/[id]`.             |

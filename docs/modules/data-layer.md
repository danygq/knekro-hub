...
`PUBLIC_SUPABASE_URL`, `PUBLIC_SUPABASE_ANON_KEY`, `PUBLIC_SUPABASE_PUBLISHABLE_KEY`, `SUPABASE_URL`,
`SUPABASE_ANON_KEY`, `SUPABASE_PUBLISHABLE_KEY`, `POSTGRES_HOST/USER/DATABASE`, `NEXT_PUBLIC_DEV_SUPABASE_REDIRECT_URL`.
Service role key (`.env.example`) is server-only, never client.
...
- `games` — `Game`: title, slug, description, cover_url, platforms[], status (FK→game_status), play_count, tags[],
  timestamps.

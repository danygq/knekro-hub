...
| `/games`                | `pages/games/index.astro`          | Status filter panel + global DB name search + paginated games grid | `game_status` + `games` via `lib/games.ts` (server); search via `/api/games/search` |
...
| `/api/games/search`     | `pages/api/games/search.ts`        | `GET` → global name search + pagination (JSON)                     | `q`, `offset`, `limit` query params                                                 |
...
| `GameCard.astro`    | 2:3 cover card for the grid (`data-status-ids`, `data-name`, status badge) |
...
| `pages/games.css` | `/games` filter panel + off-canvas drawer + name-search bar (moved out of the old `<style is:global>` block) | `pages/games/index.astro` frontmatter |

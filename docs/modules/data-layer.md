...
module: data-layer
owner_area: backend
last_verified_against_commit: bce2e77
depends_on: []
---
...
- `games` — **already queried by `/games`** (`loadGamesLibrary`, `lib/games.ts`): selects
  ...
  `game_status_id`, order `id`, limit 24 per page) plus an exact `count` of all rows. `vote_count`/`avg_vote` are
  maintained by the `on_vote_change` trigger (see below). Schema still unconfirmed against Supabase.
